...
def list_roles(request):
    ...
    """
    roles = RoleManagementService.get_all_roles()
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_users_in_role(request, role_name):
    """
    Get all users assigned to a specific role
    """
    users = RoleManagementService.get_users_in_role(role_name)
    if users is not None:
        # Create a simple serializer for user data
        user_data = [
            {
                'id': user.id,
